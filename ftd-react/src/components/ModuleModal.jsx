...
import { X, BookOpen, Target, Lightbulb } from 'lucide-react';
import TestModal from './TestModal';
import { getTestByModuleId } from '../data/testData';
import { useTimeTracking } from '../contexts/TimeTrackingContext';

const ModuleModal = ({ module, onClose }) => {
  const [isTestModalOpen, setIsTestModalOpen] = useState(false);
  const [currentTest, setCurrentTest] = useState(null);
  const { startModule, endModule } = useTimeTracking();

  // Start tracking when module opens
  useEffect(() => {
    if (module?.id) {
      startModule(module.id);
    }

    // Cleanup: end module tracking when modal closes
    return () => {
      endModule();
    };
  }, [module?.id, startModule, endModule]);
  ...
  const handleStartTest = () => {
    const test = getTestByModuleId(module.id);
    if (test) {
      setCurrentTest(test);
